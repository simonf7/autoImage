...
/** @var bool $boolAdm - Was the passphrase sent and correct, this is needed to run any commands */
...
if ($reqCmd == 'delete' && $boolPP === true) {
    ...
    if (!$reqFN) {
        exit('Sorry, you must specify the file you want to delete.');
    }
...
/** @var bool $maxSize - If the size request starts with an 'm' we're just choosing the maximum size */
...
if (strpos($actualFile, '//') !== false) {
    ...
    if (!file_exists($actualFile)) {
        ...
        /** @var bool $fileFound - Has a file been found? */
        $fileFound = false;
